...
root_agent = Agent(
    ...
    static_instruction=(
        ...
        "3. MANDATORY, never skip: call sector_analyst_agent. From the "
        "assessment take caen_code, the most recent year in "
        "financial_history, and that year's turnover, net_profit, and "
        "employee_count, and pass all of them in the request. Wait for its "
        "result before continuing; only proceed without it if the call "
        "itself returned an error.\n"
        "4. Call report_writer_agent with the assessment JSON, the sector "
        "analysis JSON from step 3 (or the reason it is missing), and the "
        "user's language; return its report verbatim as the final answer.\n"
